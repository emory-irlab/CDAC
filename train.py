...
tf.flags.DEFINE_string("Training_Data", "./swda_datset_training.txt", "Data source for the positive data.")
tf.flags.DEFINE_string("Test_Data", "./swda_datset_test.txt", "Data source for the positive data.")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        with open(FLAGS.glov2vec, 'r') as handle:
            g2v = pickle.load(handle)

        print len(g2v.keys()), '  ', len(vocab)
        count =0
        if FLAGS.word2vec:
        ...
        def posttopicmerging(utterance):
            ###############LITERATURE MERGING#########################
            # if re.search(".*((books)|(literature)|(literatures)|(novels)|\
            #                                            (harry potter)|(reading)|(books about)|(book about))",utterance):
            #     return '__label__literature'
        ...
        for batch in batches:
            ...
            # try:
            # print len(x_batch), "    ",  len(x_char_batch)
            ...
            if current_step % FLAGS.evaluate_every == 0:
                ...
                #     zip(x_dev, x_utt1_dev, x_utt2_dev, x_st1_dev, x_st2_dev, x_ib1_dev, x_ib2_dev, x_pos0_dev, x_pos1_dev, x_pos2_dev, x_yn1_dev, x_yn2_dev, handcraft_dev, y_dev)),
                #                                   FLAGS.batch_size,
                #                                   FLAGS.num_epochs)

                # for batch in batches:
                #     x_batch, x_utt1_batch, x_utt2_batch, x_st1_batch, x_st2_batch, x_ib1_batch, x_ib2_batch, x_pos0_batch, x_pos1_batch, x_pos2_batch, x_yn1_batch, x_yn2_batch, hand_batch, y_batch = zip(
                #         *batch)
